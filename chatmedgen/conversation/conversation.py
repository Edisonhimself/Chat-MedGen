...
CONV_VISION_chatmedgen = Conversation(
    system="",
    roles=("<s>[INST] ", " [/INST]"),
    messages=[],
    offset=2,
...
class Chat:
    ...
    def ask(self, text, conv):
        print("conv.messages:", conv.messages, "conv.roles", conv.roles)
        print("conv.messages[-1][0]:", conv.messages[-1][0], "conv.roles[0]:", conv.roles[0])
        if len(conv.messages) > 0 and conv.messages[-1][0] == conv.roles[0] \
                and conv.messages[-1][1][-6:] == '</Img>':  # last message is image.
            conv.messages[-1][1] = ' '.join([conv.messages[-1][1], text])
            print("conv.messages[-1][1]:", conv.messages[-1][1])
        else:
