...
from chatmedgen.models.base_model import BaseModel
from transformers import StoppingCriteria, StoppingCriteriaList

from chatmedgen.conversation.conversation import StoppingCriteriaSub

# from chatmedgen.models.peft.prefix_tuning import PrefixEncoder, get_prompt_prefix_tuning           


class MiniGPTBase(BaseModel):
    """
    Base class for Chat-Medgen
    """
    ...
    def get_context_emb_only_img(self, prompt, img_list):
        # device = img_list[0].device
        # prompt_segs = prompt.split('<ImageHere>')
        # assert len(prompt_segs) == len(img_list) + 1, "Unmatched numbers of image placeholders and images."
        # seg_tokens = [
    ...
    def forward(self, samples, reduction='mean'):
        ...
        # if self.peft_config.prefix_tuning.is_use == True:
        #     past_key_values = self.get_prompt_prefix_tuning(batch_size=inputs_embeds.shape[0], device=inputs_embeds.device, 
        #                                                config=self.peft_config.prefix_tuning, dtype=inputs_embeds.dtype)
        #     prefix_attention_mask = torch.ones(inputs_embeds.shape[0], self.peft_config.prefix_tuning.pre_seq_len).to(inputs_embeds.device)
